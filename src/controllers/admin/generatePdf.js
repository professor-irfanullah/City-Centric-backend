...
const generatePdf = async (req, res, next) => {
  ...
  try {
    ...
    // FIX: Set up request interception AFTER page is created but before navigation
    // But we need to be careful about timing
    ...
    // FIX: Set up request interception AFTER the page is ready
    // But we need to do it before setting content

    // First, set request interception
    await page.setRequestInterception(true);

    // Then handle requests
    page.on('request', (request) => {
      // Only abort resources that aren't critical for PDF rendering
      if (['font', 'stylesheet', 'image', 'media'].includes(request.resourceType())) {
        request.abort();
      } else {
        request.continue();
      }
    ...
    // FIX: Set content with a longer timeout and different waitUntil option
    await page.setContent(html, {
      waitUntil: 'domcontentloaded', // Changed from 'networkidle0' to avoid timeout issues
      timeout: 60000 // Increased timeout
    });

    // Optional: Wait a bit for any JavaScript to execute
    ...
    const pdfBuffer = await page.pdf({
      ...
      preferCSSPageSize: true // Helps with consistent sizing
    });
